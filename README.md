...
- `evaluation/` Evaluation inputs/outputs and table artifacts.
  ...
  - `human-secondary/` Manual evaluation results (secondary evaluator).
  - `audit/` Missing-value (null) check results for the primary and secondary evaluations.
  - `human-mean/` Mean scores of the primary and secondary evaluations for generating tables 3–5.
  - `gpt-4o/` LLM-based evaluation results generated by GPT-4o as an evaluator.
  - `tables/` Final Tables 3–5 and their supplementary tables generated by scripts.
- `pipeline/` Shell entrypoints that run the main workflows.
  ...
  - `03_audit_evaluation.sh` Quality checking of the primary and secondary manual-evaluation records (e.g., fill missing values / fix obvious recording errors).
  ...
  - `05_build_table_3_5_icc.sh` Generate the final manuscript tables (Tables 3–5, and Supplementary Tables S1-S5), including confidence intervals and statistical tests, from the aggregated results.
...
(.venv) $ bash pipeline/05_build_table_3_5_icc.sh
```
